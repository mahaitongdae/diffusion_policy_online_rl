...
class DPMDV2Fix12345(Algorithm):
    ...
    def __init__(
        ...
        *,
        ...
        # Bug #5 fix: Monkey-patch p_sample to remove 0.5 scaling on initial noise
        # Standard DDPM starts from N(0, I), not N(0, 0.25I)
        original_p_sample = self.agent.diffusion.p_sample
        def fixed_p_sample(key: jax.Array, model, shape):
            x_key, noise_key = jax.random.split(key)
            # Fix: Remove 0.5 scaling - start from N(0, I) instead of N(0, 0.25I)
            x = jax.random.normal(x_key, shape)  # Was: 0.5 * jax.random.normal(...)
            noise = jax.random.normal(noise_key, (self.agent.diffusion.num_timesteps, *shape))

            def body_fn(x, input):
                t, noise = input
                noise_pred = model(t, x)
                model_mean, model_log_variance = self.agent.diffusion.p_mean_variance(t, x, noise_pred)
                x = model_mean + (t > 0) * jnp.exp(0.5 * model_log_variance) * noise
                return x, None

            x, _ = jax.lax.scan(body_fn, x, (jnp.arange(self.agent.diffusion.num_timesteps - 1, -1, -1), noise))
            return x

        # Apply the monkey-patch
        self.agent.diffusion.p_sample = fixed_p_sample
